...
const getStaffById = async (staff_id) => {
  ...
  const result = await request.query(`
    SELECT s.*, u.full_Name AS user_name, u.email, u.phone, d.name AS department_name, r.role_name
    FROM staff s
...
const updateStaff = async (staff_id, updates) => {
  ...
  if (updates.license_number) { request.input('license_number', sql.VarChar(20), updates.license_number); fields.push('license_number = @license_number'); }
  ...
  if (fields.length === 0) return getStaffById(staff_id);
  request.input('staff_id', sql.Int, staff_id);
  await request.query(`UPDATE staff SET ${fields.join(', ')} WHERE staff_id = @staff_id`);
  return getStaffById(staff_id);
};
